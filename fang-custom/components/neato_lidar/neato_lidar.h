...
static constexpr size_t LIDAR_POINTS_PER_SCAN = 1080;
static constexpr size_t LIDAR_BYTES_PER_POINT = 5;
static constexpr size_t LIDAR_SCAN_SIZE = LIDAR_POINTS_PER_SCAN * LIDAR_BYTES_PER_POINT;
static constexpr size_t LIDAR_RING_SIZE = 6144;   // > 5400, marge pour header
...
static constexpr uint8_t LIDAR_MAGIC_0 = 0x4C;
static constexpr uint8_t LIDAR_MAGIC_1 = 0x44;
static constexpr uint8_t LIDAR_FORMAT_VERSION = 0x01;

// Commande UART pour demander un scan lidar
static constexpr const char *CMD_GET_LDS_SCAN = "GetLDSScan\n";

class NeatoLidarComponent : public PollingComponent,
                           public uart::UARTDevice,
                           public mqtt::MQTTComponent {
 public:
  NeatoLidarComponent() : PollingComponent(100) {}   // 100 ms par défaut
  ...
  // UARTDevice : on lit dans le buffer UART
  void on_uart_data(uint8_t data) override;
 ...
 protected:
  ...
  void publish_scan(const uint8_t *data, size_t len);
